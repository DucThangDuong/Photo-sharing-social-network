...
class _DiscoverPostDetailPageState extends State<DiscoverPostDetailPage> {
        ...
        if (mounted) {
          setState(() {
            if (response is Map<String, dynamic> && response.containsKey('data')) {
              _post = HomePostDTO.fromJson(response['data']);
            } else {
              _post = HomePostDTO.fromJson(response);
            }
          });
          _fetchComments();
        }
  ...
  Future<void> _fetchComments() async {
    try {
      final response = await ApiService().get('/post/${widget.postId}/comments');
      if (mounted) {
        setState(() {
  ...
  void _goToUserProfile(int userId) {
    ...
    if (currentUser != null && userId == currentUser.id) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => GuestProfilePage(userId: userId)),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_post == null) {
      return Scaffold(
        ...
        body: const Center(child: Text('Không tìm thấy bài viết', style: TextStyle(color: Colors.grey))),
      );
    }
                    ...
                    children: [
                      IconButton(
                        icon: Icon(
                          ...
                          color: _post!.isLikedByCurrentUser ? Colors.red : Colors.white,
                        ),
                  ...
                  if (_comments.isEmpty)
                    ...
                    ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        String commentAvatar = comment.avatarUrl != null && comment.avatarUrl!.isNotEmpty
                        ...
                        return ListTile(
                          ...
                          leading: GestureDetector(
                            ...
                            child: commentAvatar.isNotEmpty
                                ? CircleAvatar(radius: 16, backgroundImage: NetworkImage(commentAvatar))
      ...
      child: Row(
        children: [
          CircleAvatar(
            ...
            child: myAvatar.isEmpty ? const Icon(Icons.person, color: Colors.white, size: 18) : null,
          ),
